...
const MessageAnalyzer = (() => {
  ...
  const isGibberish = (text) => {
    // Check repeated characters (more than 4 in a row is gibberish)
    if (/([a-z])\1{4,}/i.test(text)) return true;

    // Check long consonant strings (8+ is too much)
    const consonantMatch = text.match(/[bcdfghjklmnpqrstvwxyz]{8,}/i);
    if (consonantMatch) return true;

    // Check vowel ratio - be more lenient (messages can have more/fewer vowels than names)
    // Accept anything between 15% and 85% vowels for messages
    const vowels = (text.match(/[aeiou]/gi) || []).length;
    const consonants = (text.match(/[bcdfghjklmnpqrstvwxyz]/gi) || []).length;
    const total = vowels + consonants;

    if (total > 10) { // Only check if enough letters to be meaningful
      const vowelRatio = vowels / total;
      if (vowelRatio < 0.15 || vowelRatio > 0.85) {
        return true; // Extremely few or too many vowels
      }
    }
  ...
  const isRepetitive = (text) => {
    ...
    // Check if same word repeated too many times (only longer words)
    const wordCount = {};
    for (let word of words) {
      if (word.length > 3) { // Only count words with 4+ letters
        wordCount[word] = (wordCount[word] || 0) + 1;
      }
    }

    // If one word appears more than 50% of text (very repetitive)
    for (let word in wordCount) {
      if (wordCount[word] / words.length > 0.5) {
        return true;
      }
    }
  ...
  const hasRealContent = (text) => {
    ...
    // Must have at least 3 words (very lenient)
    const words = text.trim().split(/\s+/);
    if (words.length < 3) return false;

    // Should have some capitalization (optional - accept lowercase too)
    // This is a soft requirement
    const hasCapital = /[A-Z]/.test(text);

    // Check for minimum word length (avoid single letters)
    const avgWordLength = text.replace(/\s/g, '').length / Math.max(1, words.length);
    if (avgWordLength < 1.5) return false;
  ...
  const validateMessage = (text) => {
    ...
    if (!text || text.length < 10) {
      issues.push('Message is too short (minimum 10 characters)');
    }

    // Word count - minimum 3 words
    const words = text.trim().split(/\s+/);
    if (words.length < 3) {
      issues.push('Message must contain at least 3 words');
    }

    // Spam check - this is important
    if (isSpam(text)) {
      issues.push('Message appears to be spam');
    }

    // Gibberish check - only reject if clearly gibberish
    if (isGibberish(text)) {
      issues.push('Message contains nonsensical content');
    }

    // Repetitive check - only reject if heavily repetitive
    if (isRepetitive(text)) {
      issues.push('Message has excessive repetition');
    }

    // Content quality check - soft check, informational only
